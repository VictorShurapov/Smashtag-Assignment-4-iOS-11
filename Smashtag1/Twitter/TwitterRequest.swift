...
public class TwitterRequest
{
    ...
    public func fetch(handler: @escaping (_ results: PropertyList?) -> Void) {
        performTwitterRequest(method: "GET", handler: handler)
    }
    ...
    func performTwitterRequest(method: String, handler: @escaping (PropertyList?) -> Void) {
        ...
        if let twitterAccountId = Twitter.sharedInstance().sessionStore.session()?.userID {
            ...
            let request = client.urlRequest(withMethod: method, url: "\(TwitterURLPrefix)\(self.requestType)\(jsonExtension)", parameters: self.parameters, error: &clientError)
            performTwitterRequest(request: request, client: client, handler: handler)
            
        } else {
            // Not logged in
            Twitter.sharedInstance().logIn(completion: { (session, error) in
                if let sesh = session {
                    print("signed in as \(sesh.userName)")
                } else {
                    print("error: Access to Twitter was not granted. \(String(describing: error?.localizedDescription))")
                }
            })
        }
    }
        ...
        func performTwitterRequest(request: URLRequest, client: TWTRAPIClient, handler: @escaping (PropertyList?) -> Void) {
            //////////
            client.sendTwitterRequest(request) { (response, responseData, error) -> Void in
                ...
                if responseData != nil {
                    ...
                    if propertyListResponse == nil {
                        ...
                        propertyListResponse = error as AnyObject
                    }
                } else {
